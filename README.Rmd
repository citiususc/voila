...
evolution of an observable $X_t$ depends on a deterministic force $f(X_t)$, 
called the drift, and some random fluctuation $g(X_t)$, called the diffusion:
$$d X_t = \underbrace{f(X_t)}_{Drift}d{t} + \underbrace{\sqrt{g(X_t)}}_{Diffusion} d{W_t},$$
where $W_t$ represents a 
[Wiener process](https://en.wikipedia.org/wiki/Wiener_process).

`voila` models the drift and diffusion terms as 
[gaussian processes](http://katbailey.github.io/post/gaussian-processes-for-dummies/)
(GPs).  To cope with the computational complexity of calculating the posterior 
distribution of the GPs (which scales as  $\mathcal{O}(N ^ 3)$), the GPs 
are approximated using a small set of function points, the inducing variables. 
This inducing variables $\mathbf{f}_m$ and $\mathbf{g}_m$ are the result of 
evaluating the drift and diffusion terms at some _strategically_ located 
pseudo-inputs $\mathbf{x}_m$. The pseudo-inputs and the approximate posterior 
distributions are learnt using 
[variational inference](http://blog.evjang.com/2016/08/variational-bayes.html), which minimizes the 
Kullback-Leibler divergence between the true posterior GP distribution and 
...
Sparse Gaussian Processes** __(in review)__.
...
from a single realization of an 
unidimensional
