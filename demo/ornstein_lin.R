...
v = diffParams$v
v = -1.5
fk = sde_kernel("clamped_exp_lin_kernel", list('maxAmplitude' = uncertainty,
                                               'linAmplitude' = uncertainty/ 2 / max((x - mean(x))^ 2),
                                               'linCenter' = median(x),
                                               'lengthScales' = 1),
                1, 1e-5)
...
inferenceResults = sde_vi(targetIndex, x, h, xm, fk, sk, v, 4, relTol = 1e-4)
